...
extension Ghostty {
    ...
    class App: ObservableObject {
        ...
        private static func debugLog(_ message: String) {
            ...
            let line = "[\(timestamp)] GhosttyApp: \(message)\n"
...
#if os(macOS)
            // Set our initial focus state (guard against NSApp not being ready yet)
            if let nsApp = NSApplication.shared as NSApplication? {
        ...
        static func action(_ app: ghostty_app_t, target: ghostty_target_s, action: ghostty_action_s) -> Bool {
            // Log all actions for debugging (temporarily log ALL actions)
            debugLog("ACTION: tag=\(action.tag.rawValue) target=\(target.tag.rawValue)")
            if action.tag == GHOSTTY_ACTION_RING_BELL {
                debugLog("ACTION: RING_BELL received!")
            }
        ...
        private static func ringBell(
            _ app: ghostty_app_t,
            target: ghostty_target_s) {
            debugLog("BELL: ringBell called with target tag=\(target.tag.rawValue)")
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
            ...
            default:
                debugLog("BELL: unknown target tag!")
                assertionFailure()
            }
        }
        ...
        private static func commandFinished(
            _ app: ghostty_app_t,
            target: ghostty_target_s) {
            debugLog("COMMAND_FINISHED: called with target tag=\(target.tag.rawValue)")
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                debugLog("COMMAND_FINISHED: app target - doing nothing")
                return

            case GHOSTTY_TARGET_SURFACE:
                guard let surface = target.target.surface else {
                    debugLog("COMMAND_FINISHED: surface is nil!")
                    return
                }
                guard let surfaceView = self.surfaceView(from: surface) else {
                    debugLog("COMMAND_FINISHED: surfaceView is nil!")
                    return
                }
                debugLog("COMMAND_FINISHED: Posting .ghosttyCommandFinished notification for surface")
                NotificationCenter.default.post(
            ...
            default:
                debugLog("COMMAND_FINISHED: unknown target tag!")
                assertionFailure()
            }
        }
        ...
        private static func showDesktopNotification(
            ...
            // Log to debug file
            debugLog("DESKTOP_NOTIFICATION received!")
