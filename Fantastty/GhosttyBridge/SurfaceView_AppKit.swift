...
enum AttachedTmuxInputEncoder {
    ...
    static func inputData(
        ...
        if let eventCharacters,
           ...
           scalar.value < 0x20 || scalar.value == 0x7f {
            // When shift or option is held, defer to escapeSequence so the
            // modifier is preserved via CSI u encoding.
    ...
    static func escapeSequence(
        ...
        if let eventCharacters,
           eventCharacters.count == 1,
           let scalar = eventCharacters.unicodeScalars.first,
           let seq = functionKeyScalarSequence(scalar.value, modifier: mod) {
            return seq
        }
        ...
        if let seq = keyCodeSequence(keyCode, modifier: mod) {
            return seq
        }
    ...
    private static func functionKeyScalarSequence(_ scalar: UInt32, modifier mod: Int) -> Data? {
        switch scalar {
        // Arrow keys: \e[1;<mod>A/B/C/D or \e[A/B/C/D
        case 0xF700: return arrowSequence(0x41, modifier: mod) // Up
        case 0xF701: return arrowSequence(0x42, modifier: mod) // Down
        case 0xF703: return arrowSequence(0x43, modifier: mod) // Right
        case 0xF702: return arrowSequence(0x44, modifier: mod) // Left

        // Home/End: \e[1;<mod>H/F or \eOH/\eOF
        case 0xF729: return homeEndSequence(0x48, modifier: mod) // Home
        case 0xF72B: return homeEndSequence(0x46, modifier: mod) // End
    ...
    private static func keyCodeSequence(_ keyCode: UInt16, modifier mod: Int) -> Data? {
        switch keyCode {
        // Arrow keys (fallback when eventCharacters unavailable)
        case 126: return arrowSequence(0x41, modifier: mod) // Up
        case 125: return arrowSequence(0x42, modifier: mod) // Down
        case 124: return arrowSequence(0x43, modifier: mod) // Right
        case 123: return arrowSequence(0x44, modifier: mod) // Left

        // Home/End
        case 115: return homeEndSequence(0x48, modifier: mod) // Home
        case 119: return homeEndSequence(0x46, modifier: mod) // End
    ...
    /// Arrow key: `\e[1;<mod>A` or `\e[A`
    private static func arrowSequence(_ letter: UInt8, modifier mod: Int) -> Data {
        if mod > 0 {
            return Data([0x1b, 0x5b, 0x31, 0x3b] + Array(String(mod).utf8) + [letter])
        }
        return Data([0x1b, 0x5b, letter])
    }

    /// Home/End: `\e[1;<mod>H` or `\eOH`
    private static func homeEndSequence(_ letter: UInt8, modifier mod: Int) -> Data {
        if mod > 0 {
            return Data([0x1b, 0x5b, 0x31, 0x3b] + Array(String(mod).utf8) + [letter])
        }
        // Unmodified Home/End use SS3: \eOH / \eOF
...
extension Ghostty {
    ...
    class SurfaceView: OSView, ObservableObject, Codable, Identifiable {
        ...
        override func keyDown(with event: NSEvent) {
            ...
            // On any keyDown event we unset our bell state and clear attention
            bell = false
            NotificationCenter.default.post(name: .ghosttyDidReceiveKeyInput, object: self)
